...
const formats = [
	...
	},
	{
		...
		targets: {
			...
			browsers: ["Edge >= 16", "Firefox >= 60", "Chrome >= 64", "Safari >= 11"],
		},
	},
	{
...
async function doBundle(format) {
	const result = await esbuild.build({
		...
		inject: format.bundleExternals ? ["./browser/process-shim.mjs"] : [],
...
async function doBuild() {
	...
	for (let index = 0; index < formats.length; index++) {
		...
		console.log(
			"\x1b[A\x1b[G\x1b[2KBuilding %d of %d: %s",
			index + 1,
			formats.length,
			format.name,
		);
