    ...
    reload(force, originalCb) {
        ...
        this.api.request({ a: "f", c: 1 }, (err, response) => {
            ...
            this.shareKeys = response.ok.reduce((shares, share) => {
                ...
                const auth = this.aes.encryptECB(
                    Buffer.from(handler + handler),
                );
            ...
            for (let file of response.f) {
                ...
                if (response.ph !== undefined) {
                    file.shareId = response.ph.find(
                        (item) => item.h === file.nodeId,
                    )?.ph;
                    ...
                    o.t.f.forEach((f) => {
                        ...
                        if (file) {
                            ...
                            oldparent.children.splice(
                                oldparent.children.indexOf(file),
                                1,
                            );
    ...
    getAccountInfo(originalCb) {
        ...
        this.api.request(
            ...
            (err, response) => {
                ...
                account.downloadBandwidthTotal =
                    response.mxfer || Math.pow(1024, 5) * 10;
