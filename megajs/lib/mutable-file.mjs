...
export class MutableFile extends File {
    ...
    upload(opt, source, originalCb) {
        ...
        if (
            ...
            throw Error(
                "Specify a file size or set allowUploadBuffering to true",
            );
        }
        ...
        const handleData = () => {
            while (true) {
                ...
                if (chunkPos >= chunkSize) {
                    ...
                    remainingBuffer = data.slice(
                        data.length - (chunkPos - chunkSize),
                    );
                    sendChunk();
                    break;
                }
        ...
        source.on("end", () => {
            if (size && sizeCheck !== size) {
                stream.emit(
                    "error",
                    Error(
                        "Specified data size does not match: " +
                            size +
                            " !== " +
                            sizeCheck,
                    ),
    ...
    importFile(sharedFile, originalCb) {
        ...
        if (typeof sharedFile === "string")
            sharedFile = File.fromURL(sharedFile);
